                        ...
                        const float *X, const float *Y, const float *F)
{
    ...
    float bottom = F[GetPointIndex(bl, dims)] + ( ( ( pt[0] - X[bl[0]] ) / ( X[br[0]] - X[bl[0]] ) )    *    (F[GetPointIndex(br, dims)] - F[GetPointIndex(bl, dims)] ) );
    float top =  F[GetPointIndex(tl, dims)] +   ( ( ( pt[0] - X[tl[0]] ) / ( X[tri[0]] - X[tl[0]] ) )  *  (F[GetPointIndex(tri, dims)] - F[GetPointIndex(tl, dims)] ) );
    float by = Y[bl[1]] + ((( pt[0] - X[bl[0]] ) / ( X[br[0]] - X[bl[0]] ) ) * ( Y[br[1]] - Y[bl[1]] ));
    float ty = Y[tl[1]] + ((( pt[0] - X[tl[0]] ) / ( X[tri[0]] - X[tl[0]] ) ) * ( Y[tri[1]] - Y[tl[1]] ));
    float mid = bottom + ((( pt[1] - by ) / (ty - by) ) * ( top - bottom ));
                   ...
                   int cellId, float *bbox)
{
    ...
    if(cellId >= GetNumberOfCells(dims)){
        ...
        int logCell [2] = { };
        GetLogicalCellIndex(logCell, cellId, dims);
        bbox[0] = X[logCell[0]];
        bbox[1] = X[logCell[0]+1];
        bbox[2] = Y[logCell[1]];
        bbox[3] = Y[logCell[1]+1];
    }
}
                             ...
                             const float *F)
{
	...
	//IMPLEMENTED BY JACK
    int n = GetNumberOfCells(dims);
    int count = 0;
    for(int i = 0; i < n; i++){
        
        int logCell [2] = { };
        GetLogicalCellIndex(logCell, i, dims);
        int logCell2 [2] = {logCell[0]+1, logCell[1]};
        int logCell3 [2] = {logCell[0], logCell[1]+1};
        int logCell4 [2] = {logCell[0]+1, logCell[1]+1};
        if( ((F[GetPointIndex(logCell, dims)] < 0) || (F[GetPointIndex(logCell2, dims)] < 0) || (F[GetPointIndex(logCell3, dims)] < 0) || (F[GetPointIndex(logCell4, dims)] < 0)) &&
            ((F[GetPointIndex(logCell, dims)] > 0) || (F[GetPointIndex(logCell2, dims)] > 0) || (F[GetPointIndex(logCell3, dims)] > 0) || (F[GetPointIndex(logCell4, dims)] > 0))){
